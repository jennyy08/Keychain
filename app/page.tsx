...
export default function Home() {
  ...
  const [saved, setSaved] = useState(false); const [showCorrections, setShowCorrections] = useState(false);
  const [libraryQuery, setLibraryQuery] = useState(""); const [libraryFilter, setLibraryFilter] = useState<"all" | "compatible">("all");
  ...
  const updateFile = (track: "A" | "B", file: File | null) => { if (track === "A") setFileA(file); else setFileB(file); setResultA(null); setResultB(null); setError(null); setCopied(false); setSaved(false); setShowCorrections(false); };
  const analyze = async () => {
    if (!fileA || !fileB) return; setAnalyzing(true); setError(null); setResultA(null); setResultB(null); setCopied(false); setSaved(false); setShowCorrections(false);
    try {
  ...
  const reset = () => { setFileA(null); setFileB(null); setResultA(null); setResultB(null); setError(null); setCopied(false); setSaved(false); setShowCorrections(false); };
  const swap = () => { const nextA = fileB; setFileB(fileA); setFileA(nextA); setResultA(null); setResultB(null); setError(null); setCopied(false); setSaved(false); setShowCorrections(false); };
