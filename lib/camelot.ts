...
export function camelotCompatibility(codeA: string, codeB: string): { compatible: boolean; reason: string; relationship: string | null } {
  if (codeA === "?" || codeB === "?") {
    return { compatible: false, reason: "Unknown key", relationship: null };
  }

  const numA = parseInt(codeA), letterA = codeA.slice(-1);
  const numB = parseInt(codeB), letterB = codeB.slice(-1);

  if (codeA === codeB) {
    return { compatible: true, reason: "Identical key - perfect match", relationship: "identical" };
  }
  if (numA === numB && letterA !== letterB) {
    return { compatible: true, reason: "Relative major/minor - very compatible", relationship: "relative" };
  }

  const diff = Math.min(Math.abs(numA - numB), 12 - Math.abs(numA - numB));
  if (diff === 1 && letterA === letterB) {
    return { compatible: true, reason: "Adjacent on the Camelot wheel - compatible", relationship: "adjacent" };
  }

  return { compatible: false, reason: "Not harmonically close - likely to clash", relationship: "distant" };
}

export function tempoAdjustment(bpmA: number, bpmB: number): { pctChangeNeeded: number; withinComfortableRange: boolean } {
